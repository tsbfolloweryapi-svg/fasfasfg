...
// ????????: ?????? ?????, ?????????? ???????? ????????? ?????? Payer
// ???????? ??????????? ???????: id, name, phone, tariff, discount, timeMin, date
// ???????: не менять сигнатуры методов -- используйте их для формирования списка Payers
// ????????? ??????: методы доступа - константные, простые и понятные
