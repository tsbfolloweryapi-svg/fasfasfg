...
// ¬ывод массива в виде таблицы дл€ отладки
...
// »нициализаци€ приложени€: создаем случайный вектор длиной [12,30]
...
void App::run() {
    init();
    // —охран€ем исходные данные в бинарный файл
    saveToBinary();

    // √енерируем случайный интервал [a,b]
    float a = getRand(-50.0f, 50.0f);
    float b = a + getRand(0.0f, 100.0f);
    if (a > b) swap(a, b);
    cout << "—генерирован интервал: a=" << a << " b=" << b << "\n";

    // ¬ыполн€ем шаги обработки данных по пор€дку
    step_countNegative();
    ...
    // ¬осстанавливаем исходные данные и выполн€ем дополнительные операции
    loadFromBinary();
    step_sortByAbsAscending();
    loadFromBinary();
    step_moveOutsideIntervalToEnd(a, b);

    // ‘инальный вывод состо€ни€ вектора
    loadFromBinary();
    cout << "ƒлина вектора после операций, size=" << v_.size() << "\n";
}
