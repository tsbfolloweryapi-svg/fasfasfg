...
// Вывод массива в виде таблицы для отладки
...
// Инициализация приложения: создаем случайный вектор длиной [12,30]
...
void App::run() {
    init();
    // Сохраняем исходные данные в бинарный файл
    saveToBinary();

    // Генерируем случайный интервал [a,b]
    float a = getRand(-50.0f, 50.0f);
    float b = a + getRand(0.0f, 100.0f);
    if (a > b) swap(a, b);
    cout << "Сгенерирован интервал: a=" << a << " b=" << b << "\n";

    // Выполняем шаги обработки данных по порядку
    step_countNegative();
    ...
    // Восстанавливаем исходные данные и выполняем дополнительные операции
    loadFromBinary();
    step_sortByAbsAscending();
    loadFromBinary();
    step_moveOutsideIntervalToEnd(a, b);

    // Финальный вывод состояния вектора
    loadFromBinary();
    cout << "Длина вектора после операций, size=" << v_.size() << "\n";
}
